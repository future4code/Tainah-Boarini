...
class App extends React.Component {
    state = {
      tarefas: [{
        id: Date.now(),
        texto: 'Terminar tarefas de hoje',
        completa: false
      },
      {  id: Date.now(),
         texto: 'Entregar antes das 17h',
         completa: true
      }],
      inputValue: '',
      filtro: "pendentes"
  ...
  selectTarefa = (id) => {
    const verificaTarefa = this.state.tarefas.map((tarefa) => {
      if(tarefa.completa) {
    ...
    this.setState({tarefas: verificaTarefa});
  }

  onChangeFilter = (event) => {
    const filtroSelect = event.target.value
    this.setState({filtro: filtroSelect})
  }
